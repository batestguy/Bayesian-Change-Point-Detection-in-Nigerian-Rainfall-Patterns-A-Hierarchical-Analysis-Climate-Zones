"""
MCMC script for Colab with PyMC 6 / ArviZ 1.2 API.
No package installs — assumes pymc, pytensor, arviz are already installed.
Data is embedded as CSV string.
"""
import io, os, time
...
def compute_hdi(samples, prob=0.94):
    """Compute HDI compatible with both ArviZ 0.16 and 1.2."""
    try:
...
print(f"  {len(annual_zone)} records, zones: {annual_zone['zone'].unique().tolist()}")
